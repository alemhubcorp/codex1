...
{
    protected function supportedAuthUserTypes()
    {
        return [
            'customer',
            'seller',
    ...
    public function login(Request $request)
    {
        ...
        if (Route::currentRouteName() == 'seller.login' && get_setting('vendor_system_activation') == 1) {
            ...
            $selectedUserType = 'delivery_boy';
        } elseif ($request->has('user_type')) {
            $selectedUserType = $request->get('user_type', 'customer');
        }

        if (!in_array($selectedUserType, $this->supportedAuthUserTypes())) {
            $selectedUserType = 'customer';
        }

        return view('auth.' . get_setting('authentication_layout_select') . '.user_login', [
            'selectedUserType' => $selectedUserType,
            'authUserTypes' => $this->supportedAuthUserTypes(),
        ]);
    }
    ...
    public function registration(Request $request)
    {
        ...
        return view('auth.' . get_setting('authentication_layout_select') . '.user_registration', compact('email', 'phone', 'selectedUserType'))->with([
            'authUserTypes' => $this->supportedAuthUserTypes(),
        ]);
    }
    ...
    public function dashboard()
    {
        ...
        } elseif (Auth::user()->user_type == 'logistics_company') {
            return redirect()->route('logistics.dashboard');
        } elseif (Auth::user()->user_type == 'customs_broker') {
            return redirect()->route('customs.dashboard');
    ...
    public function logistics_dashboard()
    {
        if (!Auth::check() || Auth::user()->user_type !== 'logistics_company') {
        ...
        $orderCounts = [
            'pending' => Order::whereIn('delivery_status', ['pending', 'confirmed'])->count(),
            'in_transit' => Order::whereIn('delivery_status', ['picked_up', 'on_the_way'])->count(),
            'delivered' => Order::where('delivery_status', 'delivered')->count(),
            'cancelled' => Order::where('delivery_status', 'cancelled')->count(),
        ];

        $recentOrders = Order::with([
            'user:id,name,avatar_original',
            'seller:id,name,avatar_original',
        ])->latest()->take(5)->get();

        $unreadConversations = Conversation::where(function ($query) use ($userId) {
            $query->where(function ($inner) use ($userId) {
                $inner->where('sender_id', $userId)->where('sender_viewed', 0);
            })->orWhere(function ($inner) use ($userId) {
                $inner->where('receiver_id', $userId)->where('receiver_viewed', 0);
            });
        })->count();

        $recentConversations = Conversation::with([
            'sender:id,name,avatar_original',
            'receiver:id,name,avatar_original',
        ])->where(function ($query) use ($userId) {
            $query->where('sender_id', $userId)->orWhere('receiver_id', $userId);
        })->latest()->take(5)->get();

        return view('frontend.user.logistics.dashboard', compact(
            'authUser',
            'orderCounts',
            'recentOrders',
            'unreadConversations',
    ...
    public function customs_dashboard()
    {
        if (!Auth::check() || Auth::user()->user_type !== 'customs_broker') {
            return redirect()->route('dashboard');
        }
        ...
        $preorderCounts = [
            'requested' => Preorder::where('request_preorder_status', 1)->count(),
            'prepayment' => Preorder::where('prepayment_confirm_status', 1)->count(),
            'final_order' => Preorder::where('final_order_status', 1)->count(),
            'delivered' => Preorder::where('delivery_status', 2)->where('refund_status', '!=', 2)->count(),
            'refunds' => Preorder::where('refund_status', 1)->count(),
        ];

        $recentPreorders = Preorder::with([
            'user:id,name,avatar_original',
            'preorder_product:id,product_name,product_slug,thumbnail',
        ])->latest()->take(5)->get();

        $activePreorderThreads = PreorderConversationThread::where(function ($query) use ($userId) {
            $query->where('sender_id', $userId)->orWhere('receiver_id', $userId);
        })->count();

        $recentPreorderThreads = PreorderConversationThread::with([
            'sender:id,name,avatar_original',
            'receiver:id,name,avatar_original',
            'preorderProduct:id,product_name,product_slug,thumbnail',
        ])->where(function ($query) use ($userId) {
            $query->where('sender_id', $userId)->orWhere('receiver_id', $userId);
        })->latest()->take(5)->get();

        $pendingProductQueries = PreorderProductQuery::whereNull('reply')->count();

        return view('frontend.user.customs.dashboard', compact(
            'authUser',
            'preorderCounts',
            'recentPreorders',
            'activePreorderThreads',
            'recentPreorderThreads',
            'pendingProductQueries'
        ));
    }

    public function logistics_orders(Request $request)
    {
        if (!Auth::check() || Auth::user()->user_type !== 'logistics_company') {
            return redirect()->route('dashboard');
        }
        ...
        $deliveryStatusMap = [
            ...
            'in_transit' => ['picked_up', 'on_the_way'],
            'delivered' => ['delivered'],
            'cancelled' => ['cancelled'],
        ];

        $ordersQuery = Order::with(['user:id,name,email,phone,avatar_original', 'seller:id,name,avatar_original'])->latest();

        if ($status !== 'all' && isset($deliveryStatusMap[$status])) {
            $ordersQuery->whereIn('delivery_status', $deliveryStatusMap[$status]);
        }
        ...
        if ($request->filled('date')) {
            ...
            if (count($dateRange) === 2) {
                $ordersQuery->where('created_at', '>=', date('Y-m-d', strtotime($dateRange[0])) . ' 00:00:00')
                    ->where('created_at', '<=', date('Y-m-d', strtotime($dateRange[1])) . ' 23:59:59');
            }
        }
        ...
        return view('frontend.user.logistics.orders.index', compact('orders', 'counts', 'status'));
    }

    public function logistics_order_show($id)
    {
        if (!Auth::check() || Auth::user()->user_type !== 'logistics_company') {
            return redirect()->route('dashboard');
        }

        $order = Order::with(['user', 'seller', 'orderDetails.product', 'orderDetails.pickup_point', 'commissionHistory'])->findOrFail(decrypt($id));

        return view('frontend.user.logistics.orders.show', compact('order'));
    }

    public function customs_preorders(Request $request)
    {
        if (!Auth::check() || Auth::user()->user_type !== 'customs_broker') {
            return redirect()->route('dashboard');
        }
        ...
        $status = $request->get('status', 'all');
        $query = Preorder::with(['user:id,name,email,phone,avatar_original', 'preorder_product:id,product_name,product_slug,thumbnail'])->latest();

        if ($status === 'requested') {
            $query->where('request_preorder_status', 1);
        } elseif ($status === 'accepted_requests') {
            $query->where('request_preorder_status', 2)->where('prepayment_confirm_status', 0);
        } elseif ($status === 'prepayment_requests') {
            $query->where('prepayment_confirm_status', 1);
        } elseif ($status === 'confirmed_prepayments') {
            $query->where('prepayment_confirm_status', 2)->where('final_order_status', 0);
        } elseif ($status === 'final_preorders') {
            $query->whereIn('final_order_status', [1, 2]);
        } elseif ($status === 'in_shipping') {
            $query->where('shipping_status', 2)->where('delivery_status', 0);
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($builder) use ($search) {
                $builder->where('order_code', 'like', '%' . $search . '%')
                    ...
                    ->orWhereHas('preorder_product', function ($productQuery) use ($search) {
        ...
        if ($request->filled('date')) {
            $dateRange = explode(' to ', $request->date);
            if (count($dateRange) === 2) {
                $query->where('created_at', '>=', date('Y-m-d', strtotime($dateRange[0])) . ' 00:00:00')
                    ->where('created_at', '<=', date('Y-m-d', strtotime($dateRange[1])) . ' 23:59:59');
            }
        }
        ...
        $counts = [
            'all' => Preorder::count(),
            'requested' => Preorder::where('request_preorder_status', 1)->count(),
            'accepted_requests' => Preorder::where('request_preorder_status', 2)->where('prepayment_confirm_status', 0)->count(),
            'prepayment_requests' => Preorder::where('prepayment_confirm_status', 1)->count(),
            'confirmed_prepayments' => Preorder::where('prepayment_confirm_status', 2)->where('final_order_status', 0)->count(),
            'final_preorders' => Preorder::whereIn('final_order_status', [1, 2])->count(),
            'in_shipping' => Preorder::where('shipping_status', 2)->where('delivery_status', 0)->count(),
            'delivered' => Preorder::where('delivery_status', 2)->count(),
            'refund' => Preorder::where('refund_status', 2)->count(),
        ];

        return view('frontend.user.customs.orders.index', compact('orders', 'counts', 'status'));
    }

    public function customs_preorder_show($id)
    {
        if (!Auth::check() || Auth::user()->user_type !== 'customs_broker') {
            return redirect()->route('dashboard');
        }

        $order = Preorder::with(['user', 'address', 'preorder_product.user.shop'])->findOrFail(decrypt($id));

        return view('frontend.user.customs.orders.show', compact('order'));
    }

    public function customs_product_queries(Request $request)
    {
        if (!Auth::check() || Auth::user()->user_type !== 'customs_broker') {
            return redirect()->route('dashboard');
        }

        $query = PreorderProductQuery::with(['user:id,name,email,phone,avatar_original', 'preorderProduct:id,product_name,product_slug,thumbnail'])
            ->latest();

        if ($request->filled('status') && $request->status === 'unanswered') {
            $query->whereNull('reply');
        } elseif ($request->filled('status') && $request->status === 'answered') {
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($builder) use ($search) {
                $builder->where('question', 'like', '%' . $search . '%')
                    ...
                    ->orWhereHas('preorderProduct', function ($productQuery) use ($search) {
                        $productQuery->where('product_name', 'like', '%' . $search . '%');
                    });
            });
        }

        $queries = $query->paginate(15)->appends($request->query());

        $counts = [
            'all' => PreorderProductQuery::count(),
            'unanswered' => PreorderProductQuery::whereNull('reply')->count(),
            'answered' => PreorderProductQuery::whereNotNull('reply')->count(),
        ];

        $status = $request->get('status', 'all');

        return view('frontend.user.customs.queries.index', compact('queries', 'counts', 'status'));
    }

    public function customs_product_query_show($id)
    {
        if (!Auth::check() || Auth::user()->user_type !== 'customs_broker') {
            return redirect()->route('dashboard');
        }

        $query = PreorderProductQuery::with(['user', 'preorderProduct.user.shop'])->findOrFail(decrypt($id));

        return view('frontend.user.customs.queries.show', compact('query'));
    }

    public function customs_product_query_reply(Request $request, $id)
    {
        if (!Auth::check() || Auth::user()->user_type !== 'customs_broker') {
            return redirect()->route('dashboard');
        }
        ...
        $query = PreorderProductQuery::findOrFail(decrypt($id));
        $query->reply = $request->reply;
        $query->save();

        flash(translate('Reply has been sent successfully'))->success();
        return redirect()->route('customs.product_queries.show', encrypt($query->id));
    }
