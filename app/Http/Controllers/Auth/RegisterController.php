...
class RegisterController extends Controller
{
    ...
    */
    ...
    protected function supportedUserTypes()
    {
        return [
            'customer',
            'seller',
    ...
    protected function validator(array $data)
    {
        $rules = [
            ...
            'user_type' => ['required', Rule::in($this->supportedUserTypes())],
            'g-recaptcha-response' => [
                Rule::when(get_setting('google_recaptcha') == 1 && get_setting('recaptcha_customer_register') == 1 , ['required', new Recaptcha()], ['sometimes'])
            ]
        ];

        if (($data['user_type'] ?? 'customer') === 'seller') {
            $rules['shop_name'] = 'required|string|max:200';
    ...
    protected function create(array $data)
    {
        ...
        if ($userType === 'seller') {
            ...
            $shop->slug = preg_replace('/\s+/', '-', str_replace("/", " ", $data['shop_name']));
            $shop->save();
    ...
    protected function registered(Request $request, $user)
    {
        ...
        if ($user->user_type == 'logistics_company') {
            return redirect()->route('logistics.dashboard');
        }

        if ($user->user_type == 'customs_broker') {
