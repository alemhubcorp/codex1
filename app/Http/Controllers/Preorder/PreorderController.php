...
class PreorderController extends Controller
{
    ...
    public function place_order(Request $request){
        ...
        $gst = ((($total_product_price + $tax + $shipping_cost)*$product->gst_rate)/100);
        }
        ...
        $preorder->grand_total  = $total_product_price + $tax + $shipping_cost + $gst;
        $preorder->tax  = $tax;
        if (addon_is_activated('gst_system')) {
        $preorder->gst_rate  = $product->gst_rate;
        $preorder->gst_amount  = $gst;
        }
    ...
    public function apply_coupon_code(Request $request){
        ...
        $new_total = $order->subtotal - $discount;
        $order->is_coupon_applied = 1;
        $order->coupon_discount = $discount;
        if (is_numeric($order->gst_amount) && addon_is_activated('gst_system')) {
        $order->grand_total  = (($new_total+$order->tax + $order->shipping_cost)*$order->gst_rate)/100 + $new_total;
        $order->gst_amount = (($order->grand_total * $order->gst_rate)/100);
        }else{
    ...
    public function remove_coupon_code(Request $request){
        ...
        if (is_numeric($order->gst_amount) && addon_is_activated('gst_system')) {
        $order->grand_total  = (($order->subtotal+$order->tax + $order->shipping_cost)*$order->gst_rate)/100 + $order->subtotal+ $order->tax + $order->shipping_cost;
        $order->gst_amount = (($order->grand_total * $order->gst_rate)/100);
        }else{
