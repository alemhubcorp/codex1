...
@section('content')
    ...
    <style>
        ...
        @php
            $btn_shape = get_setting('home_cta_btn_shape', 'rounded');
            $btn_size = get_setting('home_cta_btn_size', 'medium');
            
            $radius = '12px';
            if($btn_shape == 'square') $radius = '0px';
            if($btn_shape == 'pill') $radius = '50px';
        ...
        @media (max-width: 991px) {
            ...
            .cta-content-wrapper { gap: 20px; flex-direction: column; }
    ...
    @php
        $top_section = get_setting('homepage_top_section', 'slider');
    @endphp

    @if($top_section == 'slider')
    <!-- Sliders -->
            ...
            <div class="home-slider slider-full">
                @if (get_setting('home_slider_images', null, $lang) != null)
                    <div class="aiz-carousel dots-inside-bottom mobile-img-auto-height" data-autoplay="true" data-infinite="true">
                        @php
                            $slider_images_setting = \App\Models\BusinessSetting::where('type', 'home_slider_images')->where('lang', $lang)->first();
                            if(!$slider_images_setting) $slider_images_setting = \App\Models\BusinessSetting::where('type', 'home_slider_images')->first();
                            $decoded_slider_images = $slider_images_setting ? json_decode($slider_images_setting->value, true) : [];
                            
                            $sliders = [];
                            if(!empty($decoded_slider_images)){
                                $sliders = \App\Models\Upload::whereIn('id', $decoded_slider_images)
                                    ->orderByRaw("FIELD(id, " . implode(',', $decoded_slider_images) . ")")
                                    ->get();
                            }

                            $links_setting = \App\Models\BusinessSetting::where('type', 'home_slider_links')->where('lang', $lang)->first();
                            if(!$links_setting) $links_setting = \App\Models\BusinessSetting::where('type', 'home_slider_links')->first();
                            $home_slider_links = $links_setting ? json_decode($links_setting->value, true) : [];

                            $scales_setting = \App\Models\BusinessSetting::where('type', 'home_slider_scales')->where('lang', $lang)->first();
                            if(!$scales_setting) $scales_setting = \App\Models\BusinessSetting::where('type', 'home_slider_scales')->first();
                            $home_slider_scales = $scales_setting ? json_decode($scales_setting->value, true) : [];
                        @endphp
                        @foreach ($sliders as $key => $slider)
                                <div class="carousel-box">
                                    <a href="{{ isset($home_slider_links[$key]) ? $home_slider_links[$key] : '' }}">
                                        <!-- Image -->
                                    <div class="d-block mw-100 overflow-hidden h-180px h-sm-200px h-md-250px h-lg-300px h-xl-370px">
                                        @php
                                            $scale = isset($home_slider_scales[$key]) ? $home_slider_scales[$key] / 100 : 1;
                                        @endphp
                                        <img class="img-fit-contain h-100 m-auto has-transition lazyload"
                                        style="transform: scale({{ $scale }});"
                                        src="{{ static_asset('assets/img/placeholder-rect.jpg') }}"
                                        data-src="{{ $slider ? my_asset($slider->file_name) : static_asset('assets/img/placeholder.jpg') }}"
    ...
    @else
    <!-- CTA Section -->
    <section class="home-cta-section mb-3">
        <div class="container">
            <div class="cta-content-wrapper">
                @php
                    $cta_logo = get_setting('home_cta_logo');
                @endphp
                @if($cta_logo)
                <div class="cta-logo">
                    <img src="{{ static_asset('assets/img/placeholder.jpg') }}" 
                         data-src="{{ uploaded_asset($cta_logo) }}" 
                         class="lazyload" alt="CTA Logo">
                </div>
                @endif
                <div class="cta-text">
                    <h2>{{ get_setting('home_cta_title', 'Safe Global Trade - Wholesale and Retail Trade', $lang) }}</h2>
                    <p>{{ get_setting('home_cta_desc1', 'Secure wholesale supplies for businesses', $lang) }}</p>
                    <p>{{ get_setting('home_cta_desc2', 'Products for individual customers', $lang) }}</p>
                </div>
            </div>
            <div class="cta-buttons">
                <a href="{{ get_setting('home_cta_btn_retail_link', route('search')) }}" class="cta-btn cta-btn-retail">
                    <i class="las la-shopping-cart"></i>
                    {{ get_setting('home_cta_btn_retail_label', 'Retail products', $lang) }}
                </a>
                <a href="{{ get_setting('home_cta_btn_wholesale_link', route('search').'?type=wholesale') }}" class="cta-btn cta-btn-wholesale">
                    <i class="las la-industry"></i>
                    {{ get_setting('home_cta_btn_wholesale_label', 'Wholesale goods', $lang) }}
                </a>
                <a href="{{ get_setting('home_cta_btn_seller_link', route('shops.create')) }}" class="cta-btn cta-btn-seller">
                    <i class="las la-clipboard-list"></i>
                    {{ get_setting('home_cta_btn_seller_label', 'Become a seller', $lang) }}
                </a>
            </div>
        </div>
    </section>
