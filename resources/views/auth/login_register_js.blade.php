...
<script>
    function syncAuthRegistrationFields() {
        var $typeSelect = $('select[name="user_type"]');
        var $sellerFields = $('#seller-registration-fields');

        if (!$typeSelect.length || !$sellerFields.length) {
            return;
        }

        var isSeller = $typeSelect.val() === 'seller';
        $sellerFields.toggleClass('d-none', !isSeller);

        $sellerFields.find('[data-seller-required]').each(function () {
            $(this).prop('required', isSeller);
        });
    }

    $(document).on('change', 'select[name="user_type"]', syncAuthRegistrationFields);
    $(document).ready(syncAuthRegistrationFields);
</script>
